...
async function getAdminAccessToken(): Promise<string> {
  ...
  if (!res.ok) {
    console.error(`Failed to mint Shopify Admin API token: ${res.status}`, await res.text().catch(() => ""));
...
export async function adminFetch<T>(query: string, variables?: Record<string, unknown>): Promise<T> {
  ...
  if (!res.ok || json.errors) {
    console.error("Shopify Admin API request failed:", json.errors ?? res.statusText);
